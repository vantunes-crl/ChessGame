...
TEST_F(PiecesTestSuite, test_horse)
{
    EXPECT_TRUE(Table[0][1]->play(Table, 0, 2)); //try dont move
    EXPECT_TRUE(Table[0][1]->play(Table, -1, 2)); //out size
    EXPECT_TRUE(Table[0][1]->play(Table, 0, -1)); //out size
    //EXPECT_TRUE(Table[0][1]->play(Table, 2, 2)); //atc friend right 
    EXPECT_FALSE(Table[0][1]->play(Table, 2, 0)); //move left/top
    EXPECT_TRUE(Table[2][0]->play(Table, 1, 1)); //wrong move
    EXPECT_FALSE(Table[2][0]->play(Table, 3, 2)); //move back right
    EXPECT_FALSE(Table[3][2]->play(Table, 5, 2)); //move back right   //x y
    EXPECT_FALSE(Table[5][2]->play(Table, 6, 0)); //move back right   //x y
