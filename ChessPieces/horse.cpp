...
int horse::play(Table_t &Table, Pos ToMovePos)
{
    ...
    if (ToMovePos.x < 0 || ToMovePos.x > 7 || ToMovePos.y < 0 || ToMovePos.y > 7)
        return OUT_SIZE;
    else if (ToMovePos.x == pos.x && ToMovePos.y == pos.y)
        return SAME_PLACE;
    else if (Table[ToMovePos.x][ToMovePos.y] && Table[ToMovePos.x][ToMovePos.y]->getColor() == this->getColor())
        return CANT_MOVE;
    else if (((ToMovePos.x == (pos.x - 2) && (pos.y - 1)) || (ToMovePos.x == (pos.x + 2) && (pos.y - 1) == ToMovePos.y)) || 
    ((ToMovePos.x == (pos.x + 2) && (pos.y + 1)) || (ToMovePos.x == (pos.x + 2) && (pos.y + 1) == ToMovePos.y)))
    {
        Table[ToMovePos.x][ToMovePos.y] = Table[pos.x][pos.y];
        Table[pos.x][pos.y] = nullptr;
    }
    else if (((ToMovePos.y == (pos.y - 2) && (pos.x - 1)) || (ToMovePos.y == (pos.y + 2) && (pos.x - 1) == ToMovePos.x)) || 
    ((ToMovePos.y == (pos.y + 2) && (pos.x + 1)) || (ToMovePos.y == (pos.y + 2) && (pos.x + 1) == ToMovePos.x)))
    {
        Table[ToMovePos.x][ToMovePos.y] = Table[pos.x][pos.y];
        Table[pos.x][pos.y] = nullptr;
    }
